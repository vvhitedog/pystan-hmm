...
# fit 
y = hmm_data['y']
fit_data = { 'N' : len(y), 'K' : K, 'y' : y }
fit = sm.sampling(data=fit_data, iter=1000, chains=4)

def show(fit,title=None,do_plot=False,y=None):
    ...
    print('mean mu: ', np.mean(mu,axis=0))
    print('mean theta: ', np.mean(theta,axis=0))
    sys.stdout.flush()

    if do_plot:
        if y is None:
            raise RuntimeError('`y` should be supplied when plotting')
...
show(fit,'hmm fit: predicted vs observed',do_plot=True,y=y)
plt.show()
